...
#define UPDATE_FILE_PROD "droidcam-obs-client.1.txt"
#define UPDATE_FILE_BETA "droidcam-obs-client.0.txt"
extern bool opt_updater_beta;

const BYTE DEV47APPS_SERIAL[] = {
    0x01, 0x0c, 0xde, 0x51, 0xcc, 0xa7, 0x21, 0x20,
    0x93, 0x01, 0xc7, 0x0d, 0x70, 0x22, 0xf6, 0xae,
    0 };
	...
	bool verify = true, bool wantData = false)
{
		...
		nullptr, "", nullptr, headers, &signature, 30))
	{
		...
		if (!MoveFileExW(w_file_path_tmp, w_file_path, MOVEFILE_REPLACE_EXISTING)) {
...
void AutoUpdateThread::run()
{
	...
	if (!opt_updater_beta && !VerifySignature(nullptr, DEV47APPS_SERIAL)) {
	...
	if (opt_updater_beta) {
		UPDATE_FILE = UPDATE_FILE_BETA;
		ilog("-- Using beta update channel --");
	}
	else {
		UPDATE_FILE = UPDATE_FILE_PROD;
	}
	...
	// XXX - Delete old updater exe in the updates dir
	{
		std::string file_path = updatesDir;
		file_path += "/DroidCam.Client.Update.exe";
		DeleteFileA(file_path.c_str());
	}

	// Download updater exe into %TEMP%
	if (GetTempPathA(sizeof(updatesDir), updatesDir) <= 0) {
		elog("Can't get temp folder: %d", GetLastError());
		return;
	}
